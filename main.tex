...
To provide accurate recommendation, state-of-the-art recommendation systems depend on complex machine learning models (i.e. deep neural networks) on heterogeneous data sources. 
...
Aiming to overcome this obstacle, there is growing interest in devoloping \textit{explainable recommendation} systems to provide intuitive explanations of the recommendation results. 
In particular,motivated by the success of promoting sales using manually-written recommendation reasons, recently many E-commerce sites such as Alibaba and Amazon invest heavily in providing automated recommendation explanations~\cite{}. 

%explainable recommendation: persuasiveness and effectiveness
Explanations can benefit recommender systems on a number of aspects. As defined in~\cite{Tintarev2011Designing}, there are seven categories of recommendation explanations.  
From the viewpoint of E-commerce sites, \textit{pursuasiveness} and \textit{effectiveness} are the two most desirable aspects.
\textit{Persuasiveness} is the ability to convinces users to buy the recommended items. 
\textit{Effectiveness} is the power to help users make better decisions.
As these are properties that directly associat to conversion rates, an industry-level explainable recommendation system must focus on generating persuasive and effective explanations. 

Existing explanable 
 
Persuasive explanation
%3.现有工作对可解释推荐的persuasive很少，呈现形式有图片，概念等，没人考虑用句子解释
For example, explaining how the system works can in
Pursuasive explanation 
 If users understand how the item suits under different contexts, he/she will make better decisions.  
...
%contributions
In academia, explainable recommendation belongs to a broader branch of explainable AI, which is an important and urgent issue in the whole artificial intelligence community.
