...
%overview
In this section, we describe in detail the method to generate a training corpus where an input of consumption context and an item profile is associated with an output of relevant, informative and persuasive sentence. As our system is built on an E-commerce platform, we have the fortune to access a large scale product description datasets. That is, a paragraph is associated with each product (name and attributes ). However, this data set does not guarantee that the product description is relative to any consumption context, or informative about the product, or written in a persuasive manner. 

%relevance 
We take a boolean retrieval model as our labeler to filter relevant and informative descriptions. First, we build a lexicon consisting of consumption context terms. If the output product description does not contain at least one context term or it does not contain at least one product attribute in the input, the pair is deleted from our corpus. 
 
%persuasive
The identification of persuasive descriptions is more challenging. 
On one hand, annotating such a massive amount of instances is costly. On the other hand, recognition of persuasive sentences is highly subjective. 
Therefore, we adopt ``pseudo-labeling'' by fitting these descriptions to a set of patterns derived from an external data set.
The data set is crawled from blogs that we beleve to be advertisements? 
We use a combination of a set of manually constructed heuristics and a set of automatically extracted rules.
\subsection{Manually Constructed Heuristics}


\subsection{Automatically Extracted Rules}
% Data set
